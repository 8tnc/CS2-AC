...
namespace movement
{
	void ValidateDetours(std::vector<std::string> &missing);
	bool InitDetours(std::vector<std::string> &missing);
...
{
	void Capture(const CMoveData &move)
	{
		m_vecViewAngles = move.m_vecViewAngles;
		m_vecVelocity = move.m_vecVelocity;
		m_vecAbsOrigin = move.m_vecAbsOrigin;
	}
...
public:
	...
	{
		return (this->currentMoveData && this->currentMoveData->m_TouchList.Count() > 0) || this->collidingWithWorld;
	}
...
public:
	...
	MovementSnapshot moveDataPre;
	MovementSnapshot moveDataPost;
	QAngle oldAngles;
